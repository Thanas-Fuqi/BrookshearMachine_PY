...
3. **Mapping to the screen:** The `_display` function displays ANSI blocks (`██` for 1 and `  ` for 0 from the binary of a line).

## Changes to parameters
* **cpu.ROWS:** It limits the amount of printed lines to match the "DISPLAY" rows visually.
* **cpu.COLS:** Adding a margin the left side to not overlap with the printing section.
...
* **Drawing:** A line is drawn by **OR**-ing with a value where bits set to 1 correspond to pixels that should be turned on, leaving all other bits unchanged.
